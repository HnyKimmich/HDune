...
class TurnController:
    ...
    def action_phase_player_play(self, player_id: int, card, target_region_name: str = None):
        player = self.player_manager.current_player
        if not player:
            return False, "玩家不存在"
        if self.phase != GamePhase.ACTION:
            return False, "不在行动阶段"
        if self.player_manager.players[self.current_action_player_index].id != player_id:
            return False, "不是当前玩家的行动回合"
        ...
        if target_region_name and self.map_controller:
            self.map_controller.place_envoy(target_region_name, player_id)

        
        print(f"玩家 {self.current_action_player_index} 成功打出 {card.name} 到 {target_region_name}")
    ...
    def action_phase_player_show(self, player_id: int):
