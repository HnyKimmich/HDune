...
class Bootstrapper:
    ...
    def run():
        ...
        # 回合控制器，传入 map_controller
        turn_controller = TurnController(player_manager, conflict_controller, game_rule_service, map_controller)
        turn_controller.register_current_player_changed(lambda idx: set_current_player(turn_controller.player_manager.players[idx]))

        # 启动第一回合
        turn_controller.start_round()

        # 创建 ViewModel
        hud_vm = GameHUDViewModel(turn_controller, conflict_controller)

        player_info_vms = [PlayerInfoViewModel(p) for p in players]

        # 创建 MapViewModel，传入 turn_controller
        map_vm = MapViewModel(map_controller, turn_controller)

        # 创建手牌 ViewModel（当前玩家）
        current_player = player_manager.current_player
        hand_vms = [CardViewModel(card) for card in current_player.hand_cards]
